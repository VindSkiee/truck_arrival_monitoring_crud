                        ...
                        <div class="p-4 grid grid-cols-1 md:grid-cols-3 gap-4">
                            ...
                            <div class="bg-gray-50 p-3 rounded border">
                                ...
                                <div class="info-row">
                                    ...
                                    <span
                                        class="text-sm font-medium">{{ number_format($check->empty_weight ?? 0, 0, ',', '.') }}</span>
                                </div>
                                <div class="info-row">
                                    <span class="text-xs text-gray-500 font-bold">Berat Material</span>
                                    <span
                                        class="text-sm font-medium">{{ number_format($check->total_load_weight ?? 0, 0, ',', '.') }}</span>
                                </div>
                                <div class="info-row">
                                    <span class="text-xs text-gray-500 font-bold">Berat Total</span>
                                    <span
                                        class="text-sm font-medium">{{ number_format($check->sum_empty_load_weight ?? 0, 0, ',', '.') }}</span>
                                </div>
                            </div>
                            ...
                            <div class="bg-gray-50 p-3 rounded border">
                                <div class="info-row">
                                    ...
                                    <span
                                        class="text-sm font-medium">{{ number_format($check->cargo_weight ?? 0, 0, ',', '.') }}</span>
                                </div>
                                <div class="info-row">
                                    <span class="text-xs text-gray-500 font-bold">Toleransi</span>
                                    <span
                                        class="text-sm font-medium">{{ number_format($check->tolerance_weight ?? 0, 0, ',', '.') }}</span>
                                </div>
                                <div class="info-row">
                                    <span class="text-xs text-gray-500 font-bold">Loading dock</span>
                                    <span
                                        class="text-sm font-medium {{ $check->status_process == 'finish' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800' }}">{{ $check->status_process ?? '-' }}</span>
                                </div>
                                <div class="info-row">
                                    <span class="text-xs text-gray-500 font-bold">Status</span>
                                    <span
                                        class="text-sm font-medium {{ $check->status_security == 'pass' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                            ...
                            <div class="bg-gray-50 p-3 rounded border">
                                ...
                                <form action="{{ route('security.updateTruckData', $check->id) }}" method="POST">
                                    ...
                                    <div class="mb-2">
                                        ...
                                        <input type="text" name="no_truck" value="{{ $check->no_truck ?? '' }}"
                                            class="border rounded p-2 text-sm w-full" required>
                                    </div>
                                    <div class="mb-2">
                                        <label class="block text-xs text-gray-600 mb-1">Timbang Kosong</label>
                                        <input type="number" step="0.01" name="empty_weight"
                                            value="{{ $check->empty_weight ?? 0 }}"
                                            class="border rounded p-2 text-sm w-full" required>
                                    </div>
                                    <div class="mb-2">
                                        <label class="block text-xs text-gray-600 mb-1">Timbang Isi</label>
                                        <input type="number" step="0.01" name="cargo_weight"
                                            value="{{ $check->cargo_weight ?? 0 }}"
                                            class="border rounded p-2 text-sm w-full" required>
                                    </div>
                                    <div class="mb-2">
                                    ...
                                    <button type="submit"
                                        class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded text-sm w-full">Update</button>
                                </form>
                            </div>
                        </div>
                    </div>
